...
public class SupLexer : Superpower.Tokenizer<TokenType>
{
	public static SupLexer Instance = new SupLexer();
	private int _pos;
	private int line;
	private int col;
	...
	private Token EOFToken => new Token(TokenType.End, "");
	...
	//This is hacky because i shoved the superpower tokenizer on top of my earlier code. Better to pass TextSpan through everything.
	protected override IEnumerable<Result<TokenType>> Tokenize(TextSpan span)
	{
		if (span.Source == null)
		{
			yield return Result.Empty<TokenType>(span);
		}
		_source = span.ToStringValue();
		_pos = 0;
		while (_state != TokenState.Complete && _state != TokenState.Error)
		{
			var t = NextToken();
			if (t.TokenType == TokenType.End)
			{
				_state = TokenState.Complete;
				var s = new TextSpan(_source,new Position(_pos,line,col),0);
				yield return Result.Empty<TokenType>(s);
			}
			else
			{
				var loc = new TextSpan(_source, new Position(_pos-t.Literal.Length, line, col), t.Literal.Length);
				var remainder = new TextSpan(_source, new Position(_pos, line, col),_source.Length-_pos);
				yield return Result.Value(t.TokenType,loc,remainder);
			}
		}
	}
